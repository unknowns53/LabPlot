...
- 35 °C シートを選んだまま「分布の種類」を「サイズ分布逆変換 (CONTIN-like)」に切り替えると、自己相関関数からの逆ラプラス変換で 8 nm コイル + 200 nm 凝集体の二峰構造を再構成。サイドバーの重みを Intensity → Number に切り替えると、見えるピークが大粒子から小粒子に入れ替わる挙動を観察
