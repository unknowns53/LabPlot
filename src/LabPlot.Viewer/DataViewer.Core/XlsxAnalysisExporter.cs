...
public sealed class XlsxAnalysisExporter : IAnalysisExporter
{
    public void Export(AnalysisExport data, string filePath)
    {
        ...
        var entries = data.Entries.Cast<ViewerAnalysisExportEntry>().ToArray();
        using var workbook = new XLWorkbook();
        AddSeriesSheet(workbook, data, entries);
    ...
    private static void AddSeriesSheet(
        XLWorkbook workbook,
        AnalysisExport data,
        IReadOnlyList<ViewerAnalysisExportEntry> entries)
    {
        var hasData = entries.Any(static entry => entry.Points.Count > 0);
        if (!hasData)
        {
            return;
        }
