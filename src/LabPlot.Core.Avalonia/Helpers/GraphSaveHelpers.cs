...
/// file. Avalonia 版はファイル拡張子推定を `.png` / `.svg` のフィルタ
/// インデックスから決める WPF 版 API ではなく、Avalonia の StorageProvider
/// が返す `IStorageFile.Path` 経由のシンプルな拡張子マッチに切り替えている。
/// </summary>
public static class GraphSaveHelpers
{
    ...
    /// <summary>
    /// Resolve the output format from the saved file path. Honors the
    /// extension if it is <c>.png</c> or <c>.svg</c>; otherwise falls
    /// back to PNG which is the historical default for all three apps.
    /// </summary>
    public static GraphSaveFormat GetGraphSaveFormat(string filePath)
    {
    ...
    private static void ApplyPngDpiMetadata(string filePath, int dpi)
    {
        ...
        if (!HasPngSignature(bytes)) return;
        ...
        while (offset + 12 <= bytes.Length)
        {
            ...
            if (length > int.MaxValue || offset + 12 + (int)length > bytes.Length) return;
