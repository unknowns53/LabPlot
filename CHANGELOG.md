...
- **Data Viewer: 系列ごとのグラフ種別** (`5d6c971`)。折れ線 / マーカー / 棒を
  系列単位で切り替えられるようにし、棒グラフはグループ幅の自動推定つき
- **Data Viewer: フラットな系列リストへのサイドバー再設計** (`953a65b`〜`0a3c295`)。
  「テーブル → 系列」の 2 段ナビゲーションを廃止し、全ファイル横断のフラットな
  系列一覧に統一。`DisplayOrder` を描画・凡例・自動色・エクスポート順の単一の
  源にして、行ドラッグで並べ替えできる。複数選択での一括スタイル編集、
  ダブルクリック / F2 のインライン名前変更、右軸割当のスタイルパネル移設
  (行には割当中のみバッジ表示)、サイドバーの「データ / 仕上げ」2 タブ分割を含む

### Changed

- **描画パフォーマンス改善** (Data Viewer + 全モジュール)。1000 点 × 8 系列で
  60 fps を割っていた重ね描きへの対策一式:
  - パン / ホイールズーム操作中だけアンチエイリアスを切り、操作終了時に 1 回
    だけ高品質で清書する `PlotFastModeController` を新設 (`7924e12`)。操作中は
    vsync に整列した再描画ループでフレーム間隔も安定させる (`bbdddde`)。
    GPC / Spectrum / DLS のメインプロットにも同じ仕組みを展開 (`83ce1b2`)
  - マーカー無し・X 昇順の折れ線系列は Scatter より高速な SignalXY で描画
    (`d2690cb`、8×10000 点で約 1.9 倍)
  - タイトル / 軸ラベル / 色 / 線幅 / 系列名 / 右軸範囲などの見た目だけの編集は
    プロット全再構築を経由せず既存 plottable を直接更新 (`e72817e`)。副産物として
    ズーム中に見た目を編集してもズームがリセットされなくなった
  - 中央移動平均 (平滑化) を累積和方式の O(N) に書き換え (`f1ac89e`)。大きな
    窓幅でも再描画が遅くならない

### Fixed

- Data Viewer: X 値ペアが 1 組しかないデータで棒グラフのグループ幅が潰れる
  問題を修正 (`0d3a39c`)
- Data Viewer: MarkerSize 欄が空のままフォーカスを外すと、描画上のフォールバック
  値と表示が食い違う問題を修正 (`1572ffb`)
- Data Viewer: 系列名のインライン編集中に入力欄が行の幅を押し広げる問題を修正
  (`ee90dec`)
