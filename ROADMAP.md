...
- **v1.3.5 (作業中)** 🚧: 3 モジュール横断の UI usability 一貫性整備 (PR A 着手中)。`Ctrl/Cmd + L` が GPC / Spectrum で「重ね描き切替」、DLS だけ「全選択 / 全解除」と意味衝突していたため、DLS 側を `Ctrl/Cmd + A` に再割当し F1 ヘルプも同期。DLS の複数選択 ListBox に `ListBoxItem:selected` 左 3px のアクセント帯ハイライトを追加し、「いま何の sheet/run が plot に反映されているか」を視覚的に明示。GPC / Spectrum の ファイル open catch が IOException / parse 失敗のたびに `_loadedDatasets.Clear()` で作業中グラフごと破棄していた挙動を撤去し、`Task.WhenAll` の atomic 性 (1 ファイル失敗 → 全 Task throw、partial 書込なし) を根拠に既存グラフ温存に変更。最後に「履歴をクリア」(右クリックメニュー) を `ConfirmDialog` 経由に統一し、Toast 通知のみだった DLS と Confirm 付きの GPC / Spectrum の非対称を解消
